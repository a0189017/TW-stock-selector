...
關於系統提供的「技術評分」（經歷史回測校準）：
- 此評分在「持有約 10 個交易日（1~2 週）」時鑑別度最佳：分數越高，這段期間的平均報酬與勝率越高，因此短期目標價請以 1~2 週為主要參考區間。
- 評分的優勢是「前載」的——訊號出現後越早進場、單位時間報酬越高，但會隨時間衰減；持有超過一個月後，評分本身的優勢會被大盤beta稀釋，故不宜長抱等填息式的續抱。
- 評分 < 35 的標的歷史表現明顯偏弱，系統已過濾；對評分偏低（<45）者請更保守看待，需有籌碼或題材的額外支撐才推薦。
- 評分只是量化起點，仍以你的籌碼面與題材綜合判斷為最終依據。
...
候選股票資料裡的「建議價位」欄位（建議買入區間/停損/停利目標）是系統依 ATR 與均線支撐算好的參考值——以此為基礎撰寫進場策略/停損點/短期目標，不需自己重新從KD/均線估算；但仍可依籌碼面、族群強度等你的專業判斷微調，並說明微調理由。
